...
class WorkingSet:
    ...
    def __iter__(self):
        ...
        The yield order is the order in which the item' path entries were
...
class Environment:
    ...
    def __init__(
        ...
        `search_path` should be a sequence of ``sys.path`` item.  If not
...
def register_finder(importer_type, distribution_finder):
    """Register `distribution_finder` to find distributions in sys.path item
...
def declare_namespace(packageName):
    ...
    try:
        ...
        # Track what packages are namespaces, so when new path item are added,
        # they can be updated
        _namespace_packages.setdefault(parent or None, []).append(packageName)
        _namespace_packages.setdefault(packageName, [])

        for path_item in path:
            # Ensure all the parent's path item are reflected in the child,
            # if they apply
