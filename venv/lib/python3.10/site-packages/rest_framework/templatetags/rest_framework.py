...
def items(value):
    """
    Simple filter to return the item of the dict. Useful when the dict may
    have a key 'item' which is resolved first in Django template dot-notation
    lookup.  See issue #4931
    Also see: https://stackoverflow.com/questions/15416662/django-template-loop-over-dictionary-items-with-items-as-key
    """
    if value is None:
        # `{% for k, v in value.item %}` doesn't raise when value is None or
        # not in the context, so neither should `{% for k, v in value|item %}`
        return []
    return value.items()
...
def data(value):
    ...
    As per `item` filter above, allows accessing `document.data` when
