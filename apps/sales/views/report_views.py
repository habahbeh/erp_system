...
from ..models import (
    ...
    SalespersonCommission, DiscountCampaign,
    InvoiceItem
)
...
@login_required
@permission_required_with_message('sales.view_discountcampaign')
def campaign_report(request):
    """تقرير حملات الخصومات - Discount Campaign Report"""
    ...
    stats = campaigns.aggregate(
        ...
        active_campaigns=Count('id', filter=Q(is_active=True)),
        inactive_campaigns=Count('id', filter=Q(is_active=False))
    )
    ...
    for campaign in campaigns:
        ...
        campaign_invoices = SalesInvoice.objects.filter(
            company=company,
            is_posted=True,
            date__gte=campaign.start_date,
            date__lte=campaign.end_date
        )
        ...
        # حساب الخصومات حسب نوع الحملة
        if campaign.campaign_type == 'percentage':
            # حملة نسبة مئوية - تطبق على جميع الفواتير في الفترة
            campaign_stats = campaign_invoices.aggregate(
                invoice_count=Count('id'),
                total_discount=Coalesce(Sum('discount_amount'), Decimal('0')),
                total_sales=Coalesce(Sum('total_amount'), Decimal('0'))
            )

            if campaign_stats['total_sales'] > 0:
                campaign_stats['avg_discount_percent'] = (
                    campaign_stats['total_discount'] / campaign_stats['total_sales'] * 100
                )

        elif campaign.campaign_type == 'item':
            # حملة على مواد محددة
            if campaign.items.exists():
                item_sales = InvoiceItem.objects.filter(
                    invoice__in=campaign_invoices,
                    item__in=campaign.items.all()
                )

                item_stats = item_sales.aggregate(
                    invoice_count=Count('invoice__id', distinct=True),
                    total_discount=Coalesce(Sum('discount_amount'), Decimal('0')),
                    total_sales=Coalesce(Sum('net_amount'), Decimal('0'))
                )
    ...
    # جميع الحملات للاختيار
    all_campaigns = DiscountCampaign.objects.filter(
        company=company
    ).order_by('-start_date')

    context = {
        'title': _('تقرير حملات الخصومات'),
        ...
        'breadcrumbs': [
            {'title': _('المبيعات'), 'url': reverse('core:dashboard')},
            {'title': _('التقارير'), 'url': '#'},
            {'title': _('حملات الخصومات'), 'url': ''},
        ]
    }

    return render(request, 'sales/reports/campaign_report.html', context)
