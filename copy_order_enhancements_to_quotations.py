#!/usr/bin/env python3
"""
نسخ التحسينات من order_form.html إلى quotation_form.html
...
def add_css_enhancements(quotation_content, css_additions):
    """إضافة CSS للـ quotation_form.html"""
    style_end = quotation_content.find('</style>')

    if style_end != -1:
        new_content = (
            quotation_content[:style_end] +
...
def add_stock_column_header(quotation_content):
    ...
    actions_header = quotation_content.find('<th>الإجراءات</th>')

    if actions_header != -1:
        new_content = quotation_content[:actions_header] + stock_header_html + "\n" + quotation_content[actions_header:]
        return new_content
    else:
...
def add_stock_column_body(quotation_content):
    ...
    match = re.search(actions_td_pattern, quotation_content)

    if match:
        insert_pos = match.start()
        new_content = quotation_content[:insert_pos] + stock_body_html + "\n" + quotation_content[insert_pos:]
        return new_content
    else:
...
def add_multi_branch_modal(quotation_content, modal_html):
    """إضافة Modal في نهاية quotation_form.html قبل {% endblock %}"""
    endblock_pos = quotation_content.rfind('{% endblock %}')

    if endblock_pos != -1:
        new_content = (
            quotation_content[:endblock_pos] +
...
def replace_order_with_quotation(content):
    """استبدال المراجع من order إلى quotation"""
    replacements = {
        # URLs
        'purchases:order_get_supplier_item_price_ajax': 'purchases:quotation_get_supplier_item_price_ajax',
        'purchases:order_get_item_stock_multi_branch_ajax': 'purchases:quotation_get_item_stock_multi_branch_ajax',
        'purchases:order_get_item_stock_current_branch_ajax': 'purchases:quotation_get_item_stock_current_branch_ajax',
        'purchases:order_item_search_ajax': 'purchases:quotation_item_search_ajax',
        'purchases:save_order_draft_ajax': 'purchases:save_quotation_draft_ajax',

        # Form IDs and variables
        'orderForm': 'quotationForm',
        ...
        # Comments
        'أمر الشراء': 'عرض السعر',
        'أوامر الشراء': 'عروض الأسعار',
    }
...
def main():
    """الدالة الرئيسية"""
    print("🚀 بدء نسخ التحسينات من order_form.html إلى quotation_form.html")
    print("=" * 80)

    # المسارات
    base_dir = Path(__file__).parent
    order_form_path = base_dir / 'apps/purchases/templates/purchases/orders/order_form.html'
    quotation_form_path = base_dir / 'apps/purchases/templates/purchases/quotations/quotation_form.html'
    ...
    if not quotation_form_path.exists():
        print(f"❌ خطأ: لم يتم العثور على {quotation_form_path}")
        sys.exit(1)
    ...
    print("📖 قراءة quotation_form.html...")
    with open(quotation_form_path, 'r', encoding='utf-8') as f:
        quotation_content = f.read()

    # 1. CSS
    print("\n1️⃣  استخراج CSS المتقدم...")
    css_additions = extract_css_enhancements(order_content)
    if css_additions:
        quotation_content = add_css_enhancements(quotation_content, css_additions)
        print(f"   ✅ تم إضافة {len(css_additions)} حرف CSS")

    # 2. Stock Column Header
    print("\n2️⃣  إضافة عمود الرصيد في table header...")
    quotation_content = add_stock_column_header(quotation_content)
    print("   ✅ تم إضافة عمود الرصيد في الـ header")

    # 3. Stock Column Body
    print("\n3️⃣  إضافة عمود الرصيد في table body...")
    quotation_content = add_stock_column_body(quotation_content)
    ...
    if modal_html:
        quotation_content = add_multi_branch_modal(quotation_content, modal_html)
    ...
    if js_enhancements:
        old_script_start = quotation_content.find('<script>')
        old_script_end = quotation_content.rfind('</script>') + len('</script>')

        if old_script_start != -1 and old_script_end != -1:
            quotation_content = (
                quotation_content[:old_script_start] +
                js_enhancements +
                quotation_content[old_script_end:]
            )
    ...
    # 6. استبدال order → quotation
    print("\n6️⃣  استبدال order → quotation...")
    quotation_content = replace_order_with_quotation(quotation_content)
    print("   ✅ تم استبدال جميع المراجع")

    # حفظ الملف
    print("\n💾 حفظ quotation_form.html...")
    with open(quotation_form_path, 'w', encoding='utf-8') as f:
        f.write(quotation_content)

    print("\n" + "=" * 80)
    print("✅ تم النسخ بنجاح!")
    print(f"📊 حجم الملف الجديد: {len(quotation_content):,} حرف")
    print(f"📁 الموقع: {quotation_form_path}")
    print("\n🔍 الخطوة التالية: اختبر الصفحة على http://127.0.0.1:8000/purchases/quotations/create/")
