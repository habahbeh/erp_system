#!/usr/bin/env python3
"""
نسخ التحسينات من order_form.html إلى contract_form.html
...
def add_css_enhancements(contract_content, css_additions):
    """إضافة CSS للـ contract_form.html"""
    style_end = contract_content.find('</style>')

    if style_end != -1:
        new_content = (
            contract_content[:style_end] +
...
def add_stock_column_header(contract_content):
    ...
    actions_header = contract_content.find('<th>الإجراءات</th>')

    if actions_header != -1:
        new_content = contract_content[:actions_header] + stock_header_html + "\n" + contract_content[actions_header:]
        return new_content
    else:
...
def add_stock_column_body(contract_content):
    ...
    match = re.search(actions_td_pattern, contract_content)

    if match:
        insert_pos = match.start()
        new_content = contract_content[:insert_pos] + stock_body_html + "\n" + contract_content[insert_pos:]
        return new_content
    else:
...
def add_multi_branch_modal(contract_content, modal_html):
    """إضافة Modal في نهاية contract_form.html قبل {% endblock %}"""
    endblock_pos = contract_content.rfind('{% endblock %}')

    if endblock_pos != -1:
        new_content = (
            contract_content[:endblock_pos] +
...
def replace_order_with_contract(content):
    """استبدال المراجع من order إلى contract"""
    replacements = {
        # URLs
        'purchases:order_get_supplier_item_price_ajax': 'purchases:contract_get_supplier_item_price_ajax',
        'purchases:order_get_item_stock_multi_branch_ajax': 'purchases:contract_get_item_stock_multi_branch_ajax',
        'purchases:order_get_item_stock_current_branch_ajax': 'purchases:contract_get_item_stock_current_branch_ajax',
        'purchases:order_item_search_ajax': 'purchases:contract_item_search_ajax',
        'purchases:save_order_draft_ajax': 'purchases:save_contract_draft_ajax',

        # Form IDs and variables
        'orderForm': 'contractForm',
...
def main():
    """الدالة الرئيسية"""
    print("🚀 بدء نسخ التحسينات من order_form.html إلى contract_form.html")
    print("=" * 80)

    # المسارات
    base_dir = Path(__file__).parent
    order_form_path = base_dir / 'apps/purchases/templates/purchases/orders/order_form.html'
    contract_form_path = base_dir / 'apps/purchases/templates/purchases/contracts/contract_form.html'
    ...
    if not contract_form_path.exists():
        print(f"❌ خطأ: لم يتم العثور على {contract_form_path}")
        sys.exit(1)
    ...
    print("📖 قراءة contract_form.html...")
    with open(contract_form_path, 'r', encoding='utf-8') as f:
    ...
    if css_additions:
        contract_content = add_css_enhancements(contract_content, css_additions)
        print(f"   ✅ تم إضافة {len(css_additions)} حرف CSS")

    # 2. Stock Column Header
    print("\n2️⃣  إضافة عمود الرصيد في table header...")
    contract_content = add_stock_column_header(contract_content)
    print("   ✅ تم إضافة عمود الرصيد في الـ header")

    # 3. Stock Column Body
    print("\n3️⃣  إضافة عمود الرصيد في table body...")
    contract_content = add_stock_column_body(contract_content)
    ...
    if modal_html:
        contract_content = add_multi_branch_modal(contract_content, modal_html)
    ...
    if js_enhancements:
        old_script_start = contract_content.find('<script>')
        old_script_end = contract_content.rfind('</script>') + len('</script>')

        if old_script_start != -1 and old_script_end != -1:
            contract_content = (
                contract_content[:old_script_start] +
                js_enhancements +
                contract_content[old_script_end:]
            )
    ...
    # 6. استبدال order → contract
    print("\n6️⃣  استبدال order → contract...")
    contract_content = replace_order_with_contract(contract_content)
    print("   ✅ تم استبدال جميع المراجع")

    # حفظ الملف
    print("\n💾 حفظ contract_form.html...")
    with open(contract_form_path, 'w', encoding='utf-8') as f:
        f.write(contract_content)

    print("\n" + "=" * 80)
    print("✅ تم النسخ بنجاح!")
    print(f"📊 حجم الملف الجديد: {len(contract_content):,} حرف")
    print(f"📁 الموقع: {contract_form_path}")
    print("\n🔍 الخطوة التالية: اختبر الصفحة على http://127.0.0.1:8000/purchases/contracts/create/")
