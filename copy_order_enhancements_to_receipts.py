#!/usr/bin/env python3
"""
نسخ التحسينات من order_form.html إلى goods_receipt_form.html
...
def add_css_enhancements(receipt_content, css_additions):
    """إضافة CSS للـ receipt_form.html"""
    # البحث عن نهاية <style>
    style_end = receipt_content.find('</style>')

    if style_end != -1:
        # إدراج CSS قبل </style>
        new_content = (
            receipt_content[:style_end] +
...
def add_stock_column_header(receipt_content):
    ...
    actions_header = receipt_content.find('<th>الإجراءات</th>')

    if actions_header != -1:
        # إدراج قبل "الإجراءات"
        new_content = receipt_content[:actions_header] + stock_header_html + "\n" + receipt_content[actions_header:]
        return new_content
    else:
...
def add_stock_column_body(receipt_content):
    ...
    match = re.search(actions_td_pattern, receipt_content)

    if match:
        insert_pos = match.start()
        new_content = receipt_content[:insert_pos] + stock_body_html + "\n" + receipt_content[insert_pos:]
        return new_content
    else:
...
def add_multi_branch_modal(receipt_content, modal_html):
    """إضافة Modal في نهاية receipt_form.html قبل {% endblock %}"""
    endblock_pos = receipt_content.rfind('{% endblock %}')

    if endblock_pos != -1:
        new_content = (
            receipt_content[:endblock_pos] +
...
def extract_javascript_enhancements(order_content):
    ...
    # - updateStockInfo
    # - autoFillSupplierPrice (قد لا نحتاجه في receipts)
...
def replace_order_with_receipt(content):
    """استبدال المراجع من order إلى receipt"""
    replacements = {
        # URLs
        'purchases:order_get_supplier_item_price_ajax': 'purchases:receipt_get_purchase_order_item_price_ajax',
        'purchases:order_get_item_stock_multi_branch_ajax': 'purchases:receipt_get_item_stock_multi_branch_ajax',
        'purchases:order_get_item_stock_current_branch_ajax': 'purchases:receipt_get_item_stock_current_branch_ajax',
        'purchases:order_item_search_ajax': 'purchases:receipt_item_search_ajax',
        'purchases:save_order_draft_ajax': 'purchases:save_receipt_draft_ajax',

        # Form IDs and variables
        'orderForm': 'receiptForm',
        'order_id': 'receipt_id',
        'order-': 'receipt-',
        "'order'": "'receipt'",
        '"order"': '"receipt"',
        ...
        # Comments
        'أمر الشراء': 'محضر الاستلام',
        'أوامر الشراء': 'محاضر الاستلام',
    }
...
def main():
    """الدالة الرئيسية"""
    print("🚀 بدء نسخ التحسينات من order_form.html إلى goods_receipt_form.html")
    print("=" * 80)

    # المسارات
    base_dir = Path(__file__).parent
    order_form_path = base_dir / 'apps/purchases/templates/purchases/orders/order_form.html'
    receipt_form_path = base_dir / 'apps/purchases/templates/purchases/goods_receipt/goods_receipt_form.html'
    ...
    if not receipt_form_path.exists():
        print(f"❌ خطأ: لم يتم العثور على {receipt_form_path}")
        sys.exit(1)
    ...
    print("📖 قراءة goods_receipt_form.html...")
    with open(receipt_form_path, 'r', encoding='utf-8') as f:
    ...
    if modal_html:
        receipt_content = add_multi_branch_modal(receipt_content, modal_html)
    ...
    if js_enhancements:
        ...
        old_script_start = receipt_content.find('<script>')
        old_script_end = receipt_content.rfind('</script>') + len('</script>')

        if old_script_start != -1 and old_script_end != -1:
            receipt_content = (
                receipt_content[:old_script_start] +
                js_enhancements +
                receipt_content[old_script_end:]
            )
    ...
    # 6. استبدال order → receipt
    print("\n6️⃣  استبدال order → receipt...")
    receipt_content = replace_order_with_receipt(receipt_content)
    print("   ✅ تم استبدال جميع المراجع")

    # حفظ الملف
    print("\n💾 حفظ goods_receipt_form.html...")
    with open(receipt_form_path, 'w', encoding='utf-8') as f:
        f.write(receipt_content)

    print("\n" + "=" * 80)
    print("✅ تم النسخ بنجاح!")
    print(f"📊 حجم الملف الجديد: {len(receipt_content):,} حرف")
    print(f"📁 الموقع: {receipt_form_path}")
    print("\n🔍 الخطوة التالية: اختبر الصفحة على http://127.0.0.1:8000/purchases/goods-receipts/create/")
